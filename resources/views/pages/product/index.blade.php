              ...
              <div class="pb-3">
                <div class="col">
                  ...
                  for ($i=0; $i < $count_reviews; $i++) { if (Auth::id()==$reviews[$i]['user_id']) {
                    ...
                    (Auth::id()==$rates[$i]['user_id']) { $isUserComment=true; } } } @endphp <div class="col d-grid">
                    @if (!Auth::check())
      ...
      <div class="my-2">
        ...
        <div class="mb-4">
          <div class="d-flex flex-row align-items-center commented-user">
            <h5 class="mr-2">{{$comment["user_name"]}}</h5><span style="font-size:.5em; margin:0 .5rem;"><i
                class="fas fa-circle"></i></span><span class="mb-1 ml-2 small">{{$comment["time"]}}</span>
          </div>
          <div class="flex flex-row">
            <p class="mb-0" id="rate_star">
              ...
              for ($i=0; $i < $comment["rate"]; $i++) {
                ...
                } for ($i=0; $i < $comment["rate_remains"]; $i++) {
                echo '<span style="font-size:15px"><i class="fa fa-star" style="width:24px"></i></span>' ; } @endphp
                </p> </div> <div class="small" id="comment{{$comment['id']}}">
                <span>{{$comment["comment"]}}</span>
          </div>
          @if ($comment["user_id"] == Auth::id())
          <a href="#" class="btn btn-warning mt-1 small" data-bs-toggle="modal" data-bs-target="#editModal"
        ...
        <form action="{{route('addRateAndBuy', $product->id)}}" method="POST">
          ...
          <div class="row pb-3">
            <div class="col d-grid">
              <button  class="btn btn-success btn-lg" name="submit" @if (Auth::check()) value="buy"
              onclick="buy()"
               type="submit" 
               @else type="button" href="#" data-bs-toggle="modal" data-bs-target="#exampleModal" @endif>Buy</button>
            </div>
          </div>
        </form>
      </div>
    ...
    <div id="carousel-related-product">
      <div class="pb-3">
        <div class="col">
          ...
          $isUserComment =false;
          $isUserRateIt = false;
          for ($i=0; $i < $count_reviews; $i++) { if (Auth::id()==$reviews[$i]['user_id']) { $isUserComment=true; } }
            for ($i=0; $i < $count_rates; $i++) { if (Auth::id()==$rates[$i]['user_id']) { $isUserComment=true; } } }
            @endphp <div class="col d-grid">
            @if (!Auth::check())
            <button type="button" class="btn btn-success btn-lg" name="submit"
              onclick="window.location.replace('/login')">Login untuk memberikan review</button>
            @endif
        ...
        star[i].classList.add('text-warning'); rateValue.innerText=i+1; rateInput.value=i+1; } }});
         function reset() {
           ...
           { 
            ...
            function buy() {
    //           var url = "{{route('addRateAndBuy', ":commentId")}}";
    // url = url.replace(':commentId', "{{$product->id}}");
    //           $.ajax({
    //         type:'PATCH',
    //         url:url,
    //         beforeSend:function(){
